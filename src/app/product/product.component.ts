...
export class ProductComponent implements OnInit {
  ...
  shareViaInstagram(img) {
    ...
    this.socialSharing.canShareVia('instagram').then(() => {
      ...
      this.socialSharing.shareViaInstagram('Hey ! Look at the new dress I just bought from PunnuMistri app. Find more such apps at ', 'www/' + img).then(() => {
        // Success!
      }).catch(() => {
        // Error!
  ...
  shareCommon(img) {
    console.log(img);
    this.socialSharing.share('Hey ! Look at the new dress I just bought from PunnuMistri app. Find more such apps at ', 'PunnuMistri store - PunnuMistri app', 'www/' + img, 'PunnuMistri.com').then(() => {
      // Success!
    }).catch(() => {
      // Error!
    });
